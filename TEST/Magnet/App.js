...
var App = function(aSettings, aCanvas) {
	...
	function tick() {
		if (status == 1) {
			var ctext = "在线蝌蚪数：" + count;
			var cpos = "当前坐标：(" + parseInt(model.userTadpole.x) + ", " + parseInt(model.userTadpole.y) + ")";
			at = document.getElementById("server");
			at.innerHTML = "<p>" + ctext + "<br />" + cpos + "</p>";
		}
		var me = model.userTadpole;
		var mindis = Number.POSITIVE_INFINITY,
		    id = -1;
		
		for(idx in model.tadpoles) {
		    var t = model.tadpoles[idx];
		    var dx = t.x - me.x,
		        dy = t.y - me.y,
		        dis = Math.sqrt(dx*dx+dy*dy);
		    
		    if(idx != me.id && dis < mindis && t.timeSinceLastServerUpdate < 300){
		        mindis = dis;
		        id = idx;
		    }
		}
		...
		if(id != -1){
		  var t = model.tadpoles[id];
		  var dx = t.x - me.x, dy = t.y - me.y;
		  ...
		  if (mindis <= 5) {
		  	if (magnet == false) {
		  		webSocketService.sendMessage("/name "+t.name);
		  		magnet = true;
		  	}
		  	if (me.x > t.x) {
		  		me.x = me.x - 1;
		  	...
		  	if (magnet == true) {
		  		webSocketService.sendMessage("/name 磁铁");
		  		magnet = false;
		  	}
		  } else {
		  	if (magnet == true) {
	...
	app.update = function() {
	  if (messageQuota < 5 && model.userTadpole.age % 50 == 0) { messageQuota++; }
		...
		for(id in model.tadpoles) {
			count++;
			model.tadpoles[id].update(mouse, id);
		}
		...
		if (following == true && targetUser != 0) {
			if (typeof(model.tadpoles[targetUser]) != "undefined") {
				var me = model.userTadpole;
				var t = model.tadpoles[targetUser];
				var dx = t.x - me.x, dy = t.y - me.y;
	...
	app.mousedown = function(e) {
		if(e.ctrlKey) {
			...
			model.userTadpole.x = pos.x;
			model.userTadpole.y = pos.y;
	...
	app.gesturestart = function(e) {
	  e.preventDefault();
	}
	app.gestureend = function(e) {
	  e.preventDefault();
	  var oInput = $('#chat');
	  if (oInput.css("display") == "none") {
	  	oInput.val(" ");
	  	oInput.css("opacity","1");
	  	if (mobile == 1) {
	  		oInput.css("display","");
	  	}
	  	oInput.keyup();
	...
	(function(){
		...
		var xPosOffline, yPosOffline;
		xPosOffline = $.cookie('xPosOffline');
		yPosOffline = $.cookie('yPosOffline');
		if((xPosOffline != null) && (yPosOffline != null)) {
			model.userTadpole.x = parseInt(xPosOffline);
			model.userTadpole.y = parseInt(yPosOffline);
		}
